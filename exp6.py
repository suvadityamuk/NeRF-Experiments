...
import numpy as np
from bitnerf_ddp import BitNeRFDDP
import os
...
from configs import exp6_config
...
def train(
    ...
    def log_video(model, epoch_val, focal):
        ...
        for idx, theta in tqdm(
            enumerate(np.linspace(0.0, 360.0, 120, endpoint=False)), total=120
        ):
    ...
    for epoch in range(epochs):
        ...
        progress_bar = tqdm(
            enumerate(train_loader), total=len(train_loader), disable=rank != 0
        )
        ...
        if epoch % 5 == 0:
            ...
            with torch.no_grad():
                ...
                for idx, (images, rays, focal) in enumerate(valid_loader):
                    ...
                    if rank == 0:
                        pred_img = (rgb[0].detach().cpu().numpy() * 255).astype(
                            np.uint8
                        )
                        ...
                        depth_map_img = (
                            depth_map[0].detach().cpu().numpy() * 255
                        ).astype(np.uint8)
...
def execute(rank, world_size, config):
    ...
    model = BitNeRFDDP(device=rank, emb_dim=config["EMB_DIM"], config=config)
...
def run_ddp_training():
    ...
    mp.spawn(execute, args=(world_size, exp6_config), nprocs=world_size, join=True)
